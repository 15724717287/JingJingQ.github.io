...
// 6.获取scrollTop前的对象
	function scrollobj(){
		...
		body.scrollTop=0;//body为1（谷歌）    body为0（火狐）
		html.scrollTop=0;
		return obj; 
	}
...
// 7.楼层跳转
	...
	for(let i=0;i<btn.length;i++){
		btn[i].onclick=function(){
			...
			animate(sobj,{scrollTop:floor[i].offsetTop+floor[i].parentNode.parentNode.offsetTop},500);
			}
	...
	window.onscroll=function(){
		for(let i=0;i<floor.length;i++){
			if(sobj.scrollTop+0.5*CH>=floor[i].offsetTop+floor[i].parentNode.parentNode.offsetTop){
				for(let j=0;j<floor.length;j++){
					btn[j].style.background="#918888";
					// btn[j].onmouseover=function(){
					// 	btn[j].style.background="#d70b1c";
					// }
		...
		// console.log(sobj)1795
		if(sobj.scrollTop>=1036){
			if(flagx){
				flagx=false;
				flags=true;
