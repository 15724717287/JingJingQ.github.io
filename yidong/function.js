...
// btn：选项卡按钮。con是选项卡内容
	...
	function xuanxiangka(btn,con,btn1){
		for(let i=0;i<btn.length;i++){
		btn[i].onmouseover=function(){
			for(let j=0;j<con.length;j++){
				con[j].style.display="none";
				btn1[i].style.backgroundColor="#333";
			}
			btn1[i].style.backgroundColor="#e5004f";
	...
	function xuanxiangka1(btn,con,btn1){
		for(let i=0;i<btn.length;i++){
		btn[i].onmouseover=function(){
			for(let j=0;j<con.length;j++){
				con[j].style.display="none";
...
function duotu_lunbo(box1,leftBtn1,rightBtn1,wins1,n1){
	...
	function movel(){
		...
		animate(box,{left:-287},500,function(){
			flag=true;
                  for(let i=0; i<n;i++){
	...
	// 左右节点轮播
	function lrCarousel1(pic,imgBox,leftBtn,rightBtn,lis){	
		//获取轮播图
		const tu = $(pic);
		//获取轮播图盒子
		const img = $(imgBox)[0];
		...
		tu[0].style.left=0;
		//显示第一个轮播点激活的颜色
		li[0].classList.add("flooractive");
		//开始轮播
			...
			li[i].onclick=function(){
				if(flag){
					...
					for(let j = 0; j < li.length; j ++){
						...
						// li[j].style.backgroundColor=liactivebgColor;
						tu[j].style.left=imgW + "px";
						// animate(tu[j],{left:imgW},500);		
					}
					//鼠标移到的该轮播点所对应的图置于中间
					tu[i].style.left=0;
					// animate(tu[i],{left:0},500)
					//鼠标移到的该轮播点激活的颜色
					li[i].classList.add("flooractive");
					// li[i].style.backgroundColor=lisColor;
					now = i;
